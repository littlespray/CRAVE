...
class blip_global_adapter_Head(nn.Module):
    """MLP Regression Head for Video Action Recognition.
    Args:
        in_channels: input channels for MLP
        hidden_channels: hidden channels for MLP
    ...
    def __init__(self, in_channels=197, hidden_channels=128, out_chanenls=32,dropout_ratio=0.2, **kwargs):
        super().__init__()
        self.fc = nn.Conv1d(in_channels, in_channels, kernel_size=1)
        self.fc_1 = nn.Conv1d(in_channels, out_chanenls, kernel_size=1)
