...
class StreamFlowUpdateBlock(nn.Module):
    ...
    def forward(self, nets, inps, corrs, flows, attentions, T=None):
        ...
        motion_features_temporal = self.transformer_block(rearrange(motion_features, '(B T) C H W -> (B H W) T C', T=T),
                                                          HW=(H, W))
...
class StreamFlowT4(nn.Module):
    def __init__(self):
        ...
        self.ratio = 8
        ckpt = './pretrained_weights/streamflow_t4-things.pth'
    ...
    def forward_T4(self, images, iters=10, flow_init=None, upsample=True, test_mode=True):
        ...
        for itr in range(iters):
            ...
            corrs = rearrange(torch.stack([corr_fns[i](coord_1s[i]) for i in range(T - 1)], dim=1),
                              'B T C H W -> (B T) C H W')
            flows = rearrange(torch.stack([coord_1s[i] - coord_0s[i] for i in range(T - 1)], dim=1),
                              'B T C H W -> (B T) C H W')
        ...
        return rearrange(nets, '(B T) C H W -> B T C H W', B=B, T=T - 1).mean(1)

    def forward(self, images, iters=10, flow_init=None, upsample=True, test_mode=True):
        # images: B, C, T, H, W
        images = images.permute(0, 2, 1, 3, 4)  # B, T, C, H, W
        images = 2 * (images / 255.0) - 1.0
        indices = [(0, 4), (3, 7), (6, 10)]
        ...
        for (begin, end) in indices:
            ...
            if begin > t or end > t:
                continue
            # print(images[:,begin:end,...].shape)
            # print(iters)
            out.append(self.forward_T4(images[:, begin:end, ...], iters, flow_init, upsample, test_mode))
        # print(out[0].shape)
        out = torch.stack(out, dim=0).mean(0)
        return out  # B, 128, H, W


if __name__ == '__main__':
    x = torch.randn(2, 3, 4, 224, 224).cuda()
