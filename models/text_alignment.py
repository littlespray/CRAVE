...
import torch
from .head import VQAHead_cls,VARHead,VQAHead,blip_global_adapter_Head
from .backbone.blip import BLIP as BLIP
    ...
    def __init__(
        ...
        if divide_head:
            for key in backbone:
                ...
                if key not in self.backbone_preserve_keys:
                    continue
                if head_type == "blip_global_adapter_Head":
                    b = blip_global_adapter_Head()
                else:
                    in_channel = 768
                    if "in_channels" in vqa_head.keys():
